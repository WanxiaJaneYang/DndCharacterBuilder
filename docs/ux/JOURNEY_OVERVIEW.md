...
| 2+ | Runtime Flow Steps | Step order comes from the resolved flow JSON of the selected packs. For the current `srd-35e-minimal` pack, the flow is: Name -> Ability Scores -> Race -> Class -> Feat -> Skills -> Equipment -> Review. |
